...
const realizarSolicitudSoap = async (backendActual: ConfiguracionBackend, metodo: string, parametros: Record<string, string>, targetUrl?: string): Promise<string> => {
  const urlFinal = targetUrl || (backendActual as any).urlBase || 'http://209.145.48.25:8091/ROOT/CoreBancarioWS';
  const esDotNet = backendActual.id === 'soap-dotnet';
  const xml = construirEnvelope(metodo, parametros, esDotNet);

  console.log(`[SOAP] POST ${urlFinal}`);
...
export const soapService = {
  async autenticar(usuario: string, contrasena: string, backendActual: ConfiguracionBackend, targetUrl?: string): Promise<{ exito: boolean; mensaje: string }> {
    try {
  ...
  async obtenerMovimientos(cuenta: string, backendActual: ConfiguracionBackend, targetUrl?: string): Promise<{ movimientos: Movimiento[]; mensaje: string }> {
    try {
      const metodo = backendActual.id === 'soap-java' ? 'obtenerMovimientos' : 'ObtenerMovimientos';
      
      const respuestaXml = await realizarSolicitudSoap(backendActual, metodo, { cuenta }, targetUrl);
      ...
      if (esDotNet) {
        ...
        for (let i = 0; i < movimientosNodes.length; i++) {
          ...
          const cuentaVal = nodo.getElementsByTagName('cuenta')[0]?.textContent || nodo.getElementsByTagName('Cuenta')[0]?.textContent;
          const nromovVal = nodo.getElementsByTagName('nromov')[0]?.textContent || nodo.getElementsByTagName('NroMov')[0]?.textContent;
          
          if (cuentaVal || nromovVal) {
            movimientos.push({
              cuenta: cuentaVal || '',
              nromov: nromovVal || '',
        ...
        for (let i = 0; i < returnNodes.length; i++) {
          ...
          const cuentaVal = nodo.getElementsByTagName('cuenta')[0]?.textContent;
          const nromovVal = nodo.getElementsByTagName('nromov')[0]?.textContent;
  ...
  async registrarDeposito(cuenta: string, importe: string, backendActual: ConfiguracionBackend, targetUrl?: string): Promise<{ exito: boolean; mensaje: string }> {
    try {
  ...
  async registrarRetiro(cuenta: string, importe: string, backendActual: ConfiguracionBackend, targetUrl?: string): Promise<{ exito: boolean; mensaje: string }> {
    try {
  ...
  async registrarTransferencia(cuentaOrigen: string, cuentaDestino: string, importe: string, backendActual: ConfiguracionBackend, targetUrl?: string): Promise<{ exito: boolean; mensaje: string }> {
    try {
  ...
  async ping(backendActual: ConfiguracionBackend, targetUrl?: string): Promise<{ exito: boolean; mensaje: string }> {
    try {
      const metodo = backendActual.id === 'soap-java' ? 'ping' : 'Ping';
      
      const respuestaXml = await realizarSolicitudSoap(backendActual, metodo, {}, targetUrl);
