...
export const restService = {
  async autenticar(usuario: string, contrasena: string, urlBase: string): Promise<{ exito: boolean; mensaje: string }> {
    try {
      const path = urlBase.endsWith('/login') ? '' : '/login';
      const respuesta = await realizarSolicitud<any>(urlBase, path, {
        usuario,
        password: contrasena
      });
      ...
      const exitoso = 
        ...
        respuesta?.resultado?.toLowerCase() === 'ok' ||
        (respuesta && typeof respuesta === 'string' && respuesta.toLowerCase() === 'ok') ||
        respuesta?.id ||
        respuesta?.usuario;
  ...
  async registrarDeposito(cuenta: string, importe: number, urlBase: string): Promise<{ exito: boolean; mensaje: string }> {
    try {
      const datosDeposito = { cuenta, importe };
      const path = urlBase.endsWith('/deposito') ? '' : '/deposito';
      
      console.log('[DEPOSITO] Datos a enviar:', datosDeposito);
      console.log('[DEPOSITO] URL base:', urlBase);
      
      const respuesta = await realizarSolicitud<any>(urlBase, path, datosDeposito);

      console.log('[DEPOSITO] Respuesta completa:', respuesta);

      if (respuesta?.estado === 1 || respuesta?.resultado === 1 || respuesta?.mensaje?.includes('éxito') || respuesta?.success === true) {
        return {
          exito: true,
  ...
  async obtenerMovimientos(cuenta: string, urlBase: string): Promise<{ movimientos: Movimiento[]; mensaje: string }> {
    try {
      const path = urlBase.includes('/movimientos') ? `/${cuenta}` : `/movimientos/${cuenta}`;
      const respuesta = await realizarSolicitud<any>(urlBase, path);
  ...
  async registrarRetiro(cuenta: string, importe: number, urlBase: string): Promise<{ exito: boolean; mensaje: string }> {
    try {
      const path = urlBase.endsWith('/retiro') ? '' : '/retiro';
      const respuesta = await realizarSolicitud<any>(urlBase, path, {
        cuenta,
        importe
      });
  ...
  async registrarTransferencia(cuentaOrigen: string, cuentaDestino: string, importe: number, urlBase: string): Promise<{ exito: boolean; mensaje: string }> {
    try {
      const path = urlBase.endsWith('/transferencia') ? '' : '/transferencia';
      const respuesta = await realizarSolicitud<any>(urlBase, path, {
        cuentaOrigen,
        cuentaDestino,
        importe
  ...
  async ping(urlBase: string): Promise<{ exito: boolean; mensaje: string }> {
    try {
      const path = urlBase.endsWith('/ping') ? '' : '/ping';
      const respuesta = await realizarSolicitud<any>(urlBase, path);
