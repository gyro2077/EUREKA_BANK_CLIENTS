...
class SoapRepositoryImpl : IEurekaBankRepository {
    ...
    private val soapEndpointUrl = ApiClient.getSoapEndpointUrl()
    private val isDotNet = EnvironmentManager.currentServer.isDotNet

    override suspend fun ping(): Result<String> = runCatching {
        val envelope = SoapEnvelopeBuilder.buildPing(isDotNet)
        val response = soapApi.executeSoapRequest(soapEndpointUrl, envelope)
